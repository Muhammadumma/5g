...
export const DataProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const setupSync = async (tableName, setter, orderByCol = 'created_at', limitNum = 500) => {
            ...
            if (error) {
                console.warn(`Initial fetch failed for ${tableName} (might be missing ${orderByCol} column). Trying without order...`, error.message);
                const fallback = await supabase.from(tableName).select('*').limit(limitNum);
                data = fallback.data;
            }
            ...
            const channel = supabase
                .channel(`public:${tableName}-${Math.random().toString(36).substring(7)}`)
    ...
    // ── Helper to clean empty strings / undefined to null to avoid DB type errors ──
    const cleanDataForDb = (data) => {
        const cleaned = {};
        for (const [key, value] of Object.entries(data)) {
            // Convert empty strings and undefined to null; keep all other values as-is
    ...
    const addPatient = async (patientData) => {
        ...
        const capitalizedName = toTitleCase(patientData.name);

        // ── Explicit column whitelist ─────────────────────────────────────────────
        // NEVER use ...spread here. Spreading the whole formData object means any
        // extra/renamed frontend field that doesn't exist in the DB table causes:
        //   "Could not find the 'x' column in the schema cache"
        // By listing every column explicitly, unknown fields are silently ignored
        // and every unprovided value is safely stored as null.
        const record = {
            // Minimal required fields for patient registration UI
            name: `${patientData.firstName?.trim() || ''} ${patientData.surname?.trim() || ''}`.trim() || null,
            surname: patientData.surname?.trim() || null,
            firstName: patientData.firstName?.trim() || null,
            dob: patientData.dob || null,
            gender: patientData.gender || null,
            phonePrimary: patientData.phonePrimary || null,
            state: patientData.state || null,
            lga: patientData.lga || null,
            address: patientData.address || null,
            status: 'Active',
        };

        const result = await insertDoc('patients', record);
