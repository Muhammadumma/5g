...
const PatientRegistration = ({ onComplete, onCancel }) => {
    ...
    const validatePrimary = async () => {
        let errs = {};
        // Only name is strictly required — all other fields are optional and saved as null if empty
    ...
    const validateContact = async () => {
        let errs = {};
        // Email format check only if an email was actually provided
        if (formData.email && !/\S+@\S+\.\S+/.test(formData.email)) errs.email = "Invalid email";
        // Duplicate phone check only if a phone was actually provided
    ...
    const handleSave = async (another = false) => {
        ...
        try {
            ...
            delete finalData.hmoID; // Remove the old key

            const addedId = await addPatient(finalData);
            if (!addedId) {
                throw new Error("Failed to save patient to database. Check console for details.");
            }
